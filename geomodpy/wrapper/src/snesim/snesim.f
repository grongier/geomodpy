...
c           						/07-09-15/Mejer Hansen 
c
c Ch91    / Allocate array size based on input parameters
c                                               /02-04-23/Guillaume Rongier 
c                 				 
c Ch92    / Prevent deallocate when no data are used
c                                               /02-05-25/Guillaume Rongier 
c
c@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
c Ch91 new begin: remove fixed simulation grid size
c      INTEGER, PARAMETER :: MAXXYZ = 20000000
c Ch91 new end
...
c Ch30 new end
c Ch91 new begin: remove maximum number of data
      ! Maximum number of original sample data
c      INTEGER, PARAMETER :: MAXDAT=1000
c Ch91 new end
      REAL, PARAMETER :: EPSILON=1.0e-20, DEG2RAD=3.141592654/180.0
      INTEGER, PARAMETER :: UNEST=-99999

c Ch91 new begin: remove fixed TI size      
c Ch63 new begin
      ! Maximum dimensions of the training image
c      INTEGER, PARAMETER :: MAXXTR=500, MAXYTR=500, MAXZTR=500
c Ch64 new end
c Ch91 new end
...
c Ch91 new begin: remove maximum number of data
      ! Data locations
c      REAL, DIMENSION(MAXDAT) :: x, y, z
      REAL, ALLOCATABLE, DIMENSION(:) :: x, y, z
      
      ! Data values
c      INTEGER, DIMENSION(MAXDAT) :: vr
      INTEGER, ALLOCATABLE, DIMENSION(:) :: vr
c Ch91 new end
...
c      INTEGER, DIMENSION(MAXXYZ) :: order
      INTEGER, ALLOCATABLE, DIMENSION (:) :: order
      REAL, ALLOCATABLE, DIMENSION (:) :: tmp
      INTEGER, ALLOCATABLE, DIMENSION (:) :: ordercd,c,d,e,f,g,h
      REAL, ALLOCATABLE , DIMENSION  (:) :: sim
c Ch91 new end
...
c Ch91 new begin: remove fixed number of data variables
c      REAL, DIMENSION(10) :: var
...
c Ch91 new begin: remove fixed simulation grid size
      ALLOCATE(order(nxyz))
      ALLOCATE(tmp(nxyz))
      ALLOCATE(ordercd(nxyz),c(nxyz),d(nxyz),e(nxyz),f(nxyz),g(nxyz),h(nxyz))
      ALLOCATE(sim(nxyz))
c Ch91 new end
...
c Ch91 new begin: remove error if TI dimensions are higher than specified
c         IF(nxtr(imult)>MAXXTR.or.nytr(imult)>MAXYTR.or.
c     +                               nztr(imult)>MAXZTR) THEN
c            WRITE(*,*) 'ERROR: available train. grid size: ',
c     +               MAXXTR,MAXYTR, MAXZTR
c            WRITE(*,*) '       you have asked for : ',
c     +               nxtr(imult),nytr(imult),nztr(imult)
c            STOP
c         END IF
c Ch91 new end
...
!
         ...
         DO  
            ...
            IF(ioerror<0) EXIT
            nd=nd+1
c      IF(nd>MAXDAT) STOP' ERROR exceeded MAXDAT - check source file'
         END DO
         CLOSE(lin)
         ALLOCATE(x(nd),y(nd),z(nd))
         ALLOCATE(vr(nd))
         OPEN(lin,file=datafl,status='OLD')
         READ(lin,*,err=99)
         READ(lin,*,err=99) nvari
...
!
            ...
            y(k)=ymn 
            z(k)=zmn 
            IF(ixl>0) x(k)=var(ixl)
            IF(iyl>0) y(k)=var(iyl)
            IF(izl>0) z(k)=var(izl)
         END DO
         CLOSE(lin)
c Ch91 new begin: remove fixed number of data variables
         DEALLOCATE(var)
...
c Ch91 new begin: allocate TI based on input parameters
c      ALLOCATE(trainim(MAXXTR,MAXYTR,MAXZTR,nmult))
      ALLOCATE(trainim(maxval(nxtr),maxval(nytr),maxval(nztr),nmult))
c Ch91 new end
...
c Ch91 new begin: remove fixed simulation grid size
c      REAL, DIMENSION(MAXXYZ) :: tmp
c      INTEGER, DIMENSION(MAXXYZ) :: ordercd,c,d,e,f,g,h
c Ch91 new end
...
c____________________________________________________________________________
      ! Declare local variables
c Ch91 new begin: remove fixed simulation grid size
c      REAL , DIMENSION  (MAXXYZ) :: sim
c      INTEGER , DIMENSION  (MAXXYZ) :: c,d,e,f,g,h
c Ch91 new end
      INTEGER :: in
...
c Ch91 new begin: removed multiplier
c                  numcd(ix,iy,iz)=10*UNEST           
...
! Write out the realization in the output file:
	 ...
	 numncd=0
         DO iz=1, nz
            DO iy=1, ny
               DO ix=1, nx
                  WRITE(lout, '(2i9)') thres(simim(ix,iy,iz)), 
     +                                numcd(ix,iy,iz)
                  IF(numcd(ix,iy,iz)>=0) THEN 
                     totalncd=totalncd+numcd(ix,iy,iz)
                     numncd=numncd+1
...
! Finished:
      ...
      DEALLOCATE(cnodev)
c Ch91 new begin
      DEALLOCATE(order,tmp)
      DEALLOCATE(ordercd,c,d,e,f,g,h)
      DEALLOCATE(sim)
c Ch92 new begin
      IF(ALLOCATED(x)) THEN
            DEALLOCATE(x,y,z,vr)
